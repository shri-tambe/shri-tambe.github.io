...
**Graduate — 4 Semester Hours (Khoury College of Computer Sciences)**
...
| 5 | **Distributed Tracing for Agents:** OpenTelemetry architecture (spans, contexts, propagation). Designing span hierarchies for agent workflows. Causal ordering in async agent systems. Trace sampling strategies for high-volume agent traffic. Custom semantic conventions for reasoning quality. | Instrument the Week 4 system with OTel: custom span attributes for reasoning depth, tool call success rate, and confidence scores. Build a Jaeger dashboard with agent-specific views. Implement tail-based sampling. |
| 6 | **Agent Evaluation & Benchmarking:** How to measure if your agent is good. Offline evaluation, trajectory scoring, regression testing for non-deterministic systems. Pass@k vs. reliability metrics. Evaluation harness design. | Build an evaluation harness: define 20 test cases, run each 5 times, measure pass rate, latency variance, and cost per successful completion. Identify flaky behaviors. |
| 7 | **Long-Running Agents & State Machines:** Agents that run for hours/days. Checkpointing and recovery. Workflow engines vs. agent loops. Idempotency. Heartbeats and liveness detection. | Convert the multi-agent system to a long-running workflow: add checkpointing, implement crash recovery (resume from last checkpoint), build a heartbeat monitor that detects zombie agents. |
| 8 | **Architecture Review:** Teams present final project system designs. Architecture diagram, tool interfaces, failure modes identified, observability plan, evaluation strategy. Peer feedback + instructor go/no-go. | Submit: architecture doc + 5-min presentation. Receive: peer feedback + instructor approval. |
| 9 | **Failure Engineering:** Formal failure taxonomy (loops, hallucinated actions, cascade amplification, semantic drift, zombie agents). Circuit breaker patterns for non-deterministic systems. Chaos engineering methodology: fault injection, blast radius, steady-state hypothesis. | Implement a chaos testing framework: inject 5 failure types (model timeout, hallucinated tool call, infinite delegation loop, context overflow, conflicting verdicts). Measure MTTD and MTTR for each. |
| 10 | **Performance Engineering:** Token economics and cost modeling. Model routing algorithms (confidence-based cascading, task classification). Semantic caching (embedding similarity thresholds, cache invalidation). Batching and parallelism. Quantization tradeoffs. | Build a model router: classify requests → route to appropriate model. Implement semantic cache with TTL and similarity threshold tuning. Measure cost reduction vs. quality degradation curves. |
| 11 | **Agent Testing:** Unit testing agents with mocked LLMs. Deterministic replay from traces. Regression test suites for non-deterministic systems. Contract testing between agents. Snapshot testing for prompt changes. | Build a test suite: mock LLM responses for deterministic unit tests, replay production traces for integration tests, implement a regression gate that catches prompt-change regressions. |
| 12 | **Scaling Agent Systems:** Horizontal scaling patterns. Queue-based architectures (agent pools, work stealing). Rate limiting and backpressure. Graceful degradation under load. Multi-tenant agent systems. | Load-test the multi-agent system: simulate 50 concurrent requests, implement a queue-based worker pool, add backpressure, measure throughput vs. quality under load. |
| 13 | **Security & Governance:** Prompt injection attack vectors (direct, indirect, tool-mediated). Sandboxing agent actions (capability-based security). Permission models for tool access. Audit trail design for compliance. | Red-team the multi-agent system: craft 5 prompt injection attacks (direct injection, tool-result poisoning, cross-agent manipulation, privilege escalation, data exfiltration). Implement defenses for each. |
| 14 | **Final Project Work Session:** In-class working session. Instructor office hours. Peer demo feedback. Final integration and testing. | Deliverable: Working system ready for presentation. All traces, tests, and chaos experiments documented. |
